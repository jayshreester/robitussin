(function($) {
  ...
  const paralaxSelector = '.paralax-js',
    paralaxBoxes = $(paralaxSelector),
    defaultParalaxOffset = 30, //percentage
    allBoxes = [];

  $(document).ready(e => {
    paralaxBoxes.each((ind, el) => {
      let rootBox = $(el),
        bgBox = rootBox.find('.component-content').eq(0),
        paralaxSpeed = parseInt(
          rootBox
            .attr('class')
            .match(/\bspeed\-\d+$/gi)[0]
            .split('-')[1]
      ...
      bgBox
        .parallax({
          imageSrc: bgBox.attr('data-src'),
          zIndex: -100,
          speed: isNaN(paralaxSpeed) ? 0.5 : paralaxSpeed / 100,
          overScrollFix: false
        })
